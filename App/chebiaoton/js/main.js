...
function CScope() {
    ...
    this.update = function() {
        e && g ? (l += SCOPE_ACCELERATION, h -= SCOPE_ACCELERATION) : e && k ? (l += SCOPE_ACCELERATION, h += SCOPE_ACCELERATION) : f && k ? (l -= SCOPE_ACCELERATION,
            h += SCOPE_ACCELERATION) : f && g ? (l -= SCOPE_ACCELERATION, h -= SCOPE_ACCELERATION) : f ? l -= SCOPE_ACCELERATION : e ? l += SCOPE_ACCELERATION : g ? h -= SCOPE_ACCELERATION : k && (h += SCOPE_ACCELERATION);
        b.x += l;
        b.y += h;
        l *= SCOPE_FRICTION;
        h *= SCOPE_FRICTION;
...
function CMain(a) {
    ...
    this._initSounds = function() {
        createjs.Sound.initializeDefaultPlugins() && (0 < navigator.userAgent.indexOf("Opera") || 0 < navigator.userAgent.indexOf("OPR") ? (createjs.Sound.alternateExtensions = ["m4a"], createjs.Sound.addEventListener("fileload", createjs.proxy(this.soundLoaded, this)), createjs.Sound.registerSound("./sounds/ds_duck_hit.ogg", "duck_hit", 5), createjs.Sound.registerSound("./sounds/ds_duck_intro.ogg", "duck_intro", 5), createjs.Sound.registerSound("./sounds/ds_game_over.ogg", "game_over", 5), createjs.Sound.registerSound("./sounds/ds_no_bullets.ogg", "click"), createjs.Sound.registerSound("./sounds/ds_shot.ogg", "shot"), createjs.Sound.registerSound("./sounds/ds_soundtrack.ogg", "soundtrack")) :
            (createjs.Sound.alternateExtensions = ["ogg"], createjs.Sound.addEventListener("fileload", createjs.proxy(this.soundLoaded, this)), createjs.Sound.registerSound("./sounds/ds_duck_hit.m4a", "duck_hit", 5), createjs.Sound.registerSound("./sounds/ds_duck_intro.m4a", "duck_intro", 5), createjs.Sound.registerSound("./sounds/ds_game_over.m4a", "game_over", 5), createjs.Sound.registerSound("./sounds/ds_no_bullets.m4a", "click"), createjs.Sound.registerSound("./sounds/ds_shot.m4a", "shot"), createjs.Sound.registerSound("./sounds/ds_soundtrack.m4a",
                "soundtrack")), c += 6)
    };
    this._loadImages = function() {
        ...
        s_oSpriteLibrary.addSprite("hit_panel",
            "./sprites/hit_panel.png");
...
function CInterface() {
    ...
    this._init = function() {
        ...
        g.visible = !1;
        s_oStage.addChild(g);
        k = new createjs.Text("X " +
            PLAYER_LIVES, "bold 22px walibi0615bold", "#ffffff");
        k.x = 895;
        k.y = 630;
        k.textAlign = "right";
        ...
        t = new createjs.Text("0000", "bold 30px ComicSansMS-Bold",
            "#ffffff");
        t.x = 660;
        t.y = 682;
        t.textAlign = "right";
        ...
        h = [];
        v = 800;
        for (q = 0; q < NUM_BULLETS; q++) w = createBitmap(s_oSpriteLibrary.getSprite("bullet")), w.x = v, w.y = 680, s_oStage.addChild(w), h[q] = w,
            v += 30;
...
function CGame(a) {
    ...
    this.onTouchMoveMS = function(a) {
        if (!window.navigator.msPointerEnabled || a.isPrimary) a.preventDefault(), n = parseInt(((a.pageX || a.targetTouches[0].pageX) - s_oCanvasLeft) /
            s_iScaleFactor), r = parseInt(((a.pageY || a.targetTouches[0].pageY) - s_oCanvasTop) / s_iScaleFactor)
    };
    ...
    this._initPos = function() {
        for (var a = 51; a <
            CANVAS_WIDTH;) v.push(new createjs.Point(a, 565)), a += CANVAS_WIDTH / 20;
    ...
    this.initDuckObj = function() {
        ...
        if (h === DUCK_ON_SCREEN.length)
            for (var a = h = 0; a < q.length; a++) q[a].increaseSpeed();
        shuffle(C);
        shuffle(L);
    ...
    this._checkIfDuckHit = function(a) {
        if (m.bullsEye().x > a.target().x &&
            m.bullsEye().x < a.target().x + a.target().w && m.bullsEye().y > a.target().y && m.bullsEye().y < a.target().y + a.target().h) return !0
    };
    ...
    this.checkController = function() {
        ...
        H.hitTest(a.x, a.y) ? (m.moveUp(), m.moveRight(), m.downStop(), m.leftStop()) : (a = I.globalToLocal(n, r), I.hitTest(a.x, a.y) ? (m.moveDown(), m.moveRight(), m.upStop(), m.leftStop()) : (a = J.globalToLocal(n, r), J.hitTest(a.x, a.y) ? (m.moveUp(), m.moveLeft(), m.downStop(), m.rightStop()) : (a = K.globalToLocal(n, r), K.hitTest(a.x, a.y) ? (m.moveDown(), m.moveLeft(), m.upStop(), m.rightStop()) : (a = D.globalToLocal(n, r), D.hitTest(a.x, a.y) ? (m.moveUp(), m.downStop(), m.rightStop(), m.leftStop()) : (a = E.globalToLocal(n, r), E.hitTest(a.x, a.y) ?
            (m.moveDown(), m.upStop(), m.rightStop(), m.leftStop()) : (a = F.globalToLocal(n, r), F.hitTest(a.x, a.y) ? (m.moveLeft(), m.downStop(), m.rightStop(), m.upStop()) : (a = G.globalToLocal(n, r), G.hitTest(a.x, a.y) ? (m.moveRight(), m.downStop(), m.upStop(), m.leftStop()) : m.resetAllDirection())))))))
    };
    this.update = function() {
        if (B) {
...
function CEndPanel(a) {
    ...
    this._init = function(a) {
        ...
        b.textAlign = "center";
        c = new createjs.Text("", "bold 40px walibi0615bold",
            "#ffffff");
        c.x = CANVAS_WIDTH / 2;
